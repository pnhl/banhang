...
const policies = {
  shipping: {
    ...
    intro: "LOPA MARKET phối hợp với các đối tác vận chuyển uy tín để đơn hàng đến tay bạn an toàn, đúng hẹn và dễ theo dõi.",
    updated: "Cập nhật ngày 24.07.2026",
    sections: [
      ["1. Phạm vi giao hàng", "LOPA MARKET giao hàng toàn quốc, ngoại trừ một số khu vực hạn chế theo thông báo của đơn vị vận chuyển. Thời gian dự kiến được hiển thị rõ tại trang sản phẩm và bước thanh toán."],
      ...
      ["5. Kiểm tra khi nhận", "Vui lòng kiểm tra tình trạng kiện hàng, tem niêm phong và số lượng bên ngoài trước khi nhận. Nếu có dấu hiệu hư hỏng, hãy chụp ảnh và liên hệ LOPA trong vòng 24 giờ."],
    ],
  },
  returns: {
    ...
    sections: [
      ...
      ["3. Sản phẩm lỗi hoặc sai", "LOPA chịu toàn bộ chi phí vận chuyển khi sản phẩm bị lỗi, hư hỏng do vận chuyển, thiếu phụ kiện hoặc không đúng mô tả."],
  ...
  privacy: {
    ...
    intro: "LOPA tôn trọng quyền riêng tư và chỉ xử lý dữ liệu cần thiết để cung cấp, bảo vệ và cải thiện trải nghiệm mua sắm.",
    ...
    sections: [
      ...
      ["6. Đo lường và Google Analytics", "Google Analytics chỉ được kích hoạt khi mã đo lường được cấu hình và bạn chọn đồng ý phân tích. LOPA mặc định từ chối lưu trữ dữ liệu phân tích, không bật lưu trữ quảng cáo và cho phép bạn tiếp tục chỉ với dữ liệu thiết yếu."],
    ],
  },
  terms: {
    eyebrow: "QUYỀN & TRÁCH NHIỆM",
    title: "Điều khoản sử dụng",
    intro: "Điều khoản này quy định việc truy cập, mua sắm và sử dụng các dịch vụ do LOPA MARKET cung cấp.",
    updated: "Cập nhật ngày 24.07.2026",
    sections: [
      ...
      ["3. Giá và đơn hàng", "Giá, ưu đãi và tồn kho có thể thay đổi. Đơn hàng chỉ được xác nhận khi LOPA gửi thông báo chấp thuận sau bước kiểm tra cần thiết."],
      ...
      ["5. Giới hạn trách nhiệm", "LOPA nỗ lực duy trì dịch vụ liên tục nhưng không đảm bảo mọi gián đoạn nằm ngoài kiểm soát. Trách nhiệm được giới hạn trong phạm vi pháp luật cho phép."],
    ],
  },
} as const;
...
export default async function PolicyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!policy) notFound();
  return <><SiteHeader /><main className="policy-page"><header><div className="wrap"><p className="eyebrow">{policy.eyebrow}</p><h1>{policy.title}</h1><p>{policy.intro}</p><small>{policy.updated}</small></div></header><div className="policy-layout wrap"><aside><b>Trong trang này</b>{policy.sections.map(([title]) => <a key={title} href={`#${title.slice(0,1)}`}>{title}</a>)}<div><span>?</span><p><b>Cần hỗ trợ?</b><small>Đội ngũ LOPA sẵn sàng 24/7</small></p></div></aside><article>{policy.sections.map(([title, copy]) => <section id={title.slice(0,1)} key={title}><h2>{title}</h2><p>{copy}</p></section>)}<div className="policy-contact"><p className="eyebrow">VẪN CÒN THẮC MẮC?</p><h2>Chúng tôi ở đây để hỗ trợ.</h2><p>Liên hệ qua support@lopamarket.vn hoặc hotline 1900 6868 từ 8:00–22:00 mỗi ngày.</p><a href="/">Quay lại trang chủ →</a></div></article></div></main><SiteFooter /></>;
}
