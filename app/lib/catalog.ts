...
export const products: Product[] = [
  { id: 1, name: "Tai nghe chụp tai LopaSound Air", category: "Điện tử", price: 1290000, oldPrice: 1790000, rating: 4.9, sold: "2,1k", delivery: 2, badge: "BÁN CHẠY", image: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=1200&q=88", description: "Chống ồn chủ động, pin 48 giờ và đệm tai memory foam êm ái cho cả ngày dài." },
  ...
  { id: 8, name: "Điện thoại Lopa X Lite 5G", category: "Điện tử", price: 6490000, oldPrice: 7290000, rating: 4.7, sold: "860", delivery: 2, badge: "TRẢ GÓP 0%", image: "https://images.unsplash.com/photo-1511707171634-5f897ff02aa?auto=format&fit=crop&w=1200&q=88", description: "Màn hình OLED 120Hz, camera AI 50MP và kết nối 5G mạnh mẽ." },
...
const legacyProductNames: Record<string, string> = {
  "Tai nghe chụp tai NovaSound Air": "Tai nghe chụp tai LopaSound Air",
  "Điện thoại Nova X Lite 5G": "Điện thoại Lopa X Lite 5G",
};

export const normalizeProductBrand = <T extends Product>(product: T): T => {
  const name = legacyProductNames[product.name];
  return name ? { ...product, name } : product;
};

export const normalizeVoucherCode = (code: string) => {
  const normalized = code.trim().toUpperCase();
  return normalized === "NOVA50" ? "LOPA50" : normalized;
};
...
export const getManagedProducts = (): Product[] =>
  parseStoredArray<Product>(MANAGED_PRODUCTS_KEY, products).map(
...
export const getVoucherByCode = (code: string) =>
  getVouchers().find(
    (voucher) =>
      voucher.active && voucher.code === normalizeVoucherCode(code),
  );

export const getVoucherRedemptions = () =>
  parseStoredArray<VoucherRedemption>(VOUCHER_REDEMPTIONS_KEY, []).map(
    (redemption) => ({
      ...redemption,
      code: normalizeVoucherCode(redemption.code),
    }),
  );

export function getVoucherStats(code: string) {
  const redemptions = getVoucherRedemptions().filter(
    (item) => item.code === normalizeVoucherCode(code),
  );
  return {
...
export const getCart = (): CartLine[] => {
  ...
  try {
    ...
    return Array.isArray(parsed) ? parsed.map(normalizeProductBrand) : [];
  } catch {
    return [];
  }
};
